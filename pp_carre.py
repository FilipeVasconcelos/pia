...
if __name__ == "__main__":
    ...
    nn  = [3,16,32,img_taille[0]*img_taille[1]] 
    ...
    for lbl in lbls:
        ...
        #mnist.grayramp_show(img_gen)
        #print(60*"=")
        X0.append(resultat_vecteur(lbl))
        T.append ( np.reshape ( img_gen,( nn[3] ) ) )
    ...
    RN = reseau.MCP(nn,verbeux=2,verbe_periode=10,sigma=1.0,mu=0.)
    W, B = RN.gradient_descent( apprentissage, 40000, 0.1, evaluation )
    ...
    for lbl in lbls:
        img_gen = gen_carre(img_taille, lbl)
        T.append( np.reshape (img_gen,(nn[0]) ) )
        X0.append(resultat_vecteur(lbl))
