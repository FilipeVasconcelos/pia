...
\newcommand{\exercice}[1]{             %Création d'une macro ayant un paramètre
    ...
    \title{\textsc{INGE1-{S1} -- Initiation au Réseaux de Neurones artificiels }\\
    \emph{TD 1 -- Perceptron -- Apprentissage Profond (XOR) } }
...
Le perceptron est composé de deux fonctions (somme et d'activation). Il agit sur un vecteur 
d'entrée de $n$ composantes auxquelles on associe un poids $\omega_i$ et
d'un biais $\omega_0$ associé à unité de valeur 1. La fonction $\Sigma$ est telle que :
$$
y=\sum_i x_i\omega_i + \omega_0 
$$
...
Calculer pour les poids et biais donnés ($\omega_0=-\dfrac{3}{2}$, $\omega_1=1$, $\omega_2=1$). 
La sortie $y$ pour $\boldsymbol{x}=\{[0,0],[0,1],[1,0],[1,1]\}$.
Quel type de fonction décrit ce perceptron?


\question{}
Même question pour ($\omega_0=-\dfrac{1}{2}$, $\omega_1=1$, $\omega_2=1$)




\paragraph{Multi-couches perceptron}
Un réseau de neurones multi-couches est constitué d'un vecteur d'entrée $\boldsymbol{x}$ de $i$ composantes $x_i$, de
...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
    ...
    % Draw the output layer node
            \node[neuron,pin={[pin edge={-latex,black}]right:}] at (O) {$z$};
    % Connect every node in the input layer with every node in the
    % hidden layer.
    \foreach \source in {1,...,2}
    {
